...
public class ViewConfigManager
{
    ...
    public ViewConfigFile CreateDefaultConfig(DataTableModel table)
    {
        ...
        // ── 看板视图（需要单选字段）────────────────────────────
        var selectField = table.Fields.Find(f => f.Type == FieldType.Select && f.Options.Count is > 0 and <= 12)
                       ?? table.Fields.Find(f => f.Type == FieldType.Select);
        if (selectField != null)
        {
            config.Views.Add(new ViewConfig
            {
                ViewId = ViewConfig.NewId("kanban"),
